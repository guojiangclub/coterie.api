...
class ContentService
{
    ...
    public function created($input,$question_id=0)
    {
        ...
        try {
            ...
            if($content->style_type=='question' AND $question_id>0){
    ...
    public function setContentRecommendedAt($content_id,$coterie_id,$type=1){
        ...
        try {
            ...
            $time=$type==1?Carbon::now()->toDateTimeString():null;

            $num=$type==1?1:-1;

            if($content){

                    if(empty($content->recommended_at) AND $type==0) return null;

                    if($content->recommended_at AND $type==1) return null;
