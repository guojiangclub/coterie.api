...
class ContentController extends Controller
{
    ...
    public function store()
    {
        ...
        //动态@某人消息通知
        //发表动态通知
        if (isset($res->style_type) AND $res->style_type == 'default') {
        ...
        //回答问题消息通知
        if (isset($res->style_type) AND $res->style_type == 'question') {
        ...
        if ($list->count()) {
            ...
            foreach ($list as $key => $item) {
                ...
                if (request('title')) {
                    ...
                    if (strstr($list_new[$key]['title'], request('title')) == false) {
    ...
    public function setContentStickAt()
    {
        ...
        $time = request('type') == 1 ? Carbon::now()->toDateTimeString() : null;

        if (!$content) return $this->failed('');
